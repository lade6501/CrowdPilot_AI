...
const DashboardContent: React.FC = () => {
  ...
  const isReasoning = activeActions.some(act => act.status === "pending" || act.status === "review") || injectorLoading;
  const isActing = activeActions.some(act => act.status === "approved" || act.status === "auto_executed" || act.status === "resolved_by_governance");
  const isVerifying = activeActions.some(act => act.verification_status === "success" || act.verification_status === "failed");
  
  const hasReasoned = activeActions.length > 0 && !isReasoning;
  const hasActed = activeActions.length > 0 && !isActing && !isReasoning;
  const hasVerified = activeActions.length > 0 && !isVerifying && !isActing && !isReasoning;

  const [apiConfigured, setApiConfigured] = useState<boolean | null>(null);
  const [activeTab, setActiveTab] = useState<"live" | "studio" | "retro">("live");
    ...
    <div className="min-h-screen pb-12">
        ...
        <div className="max-w-7xl mx-auto w-full flex flex-col xl:flex-row items-center justify-between gap-4">
          ...
          {stadiumState?.match && (
            <div className="flex items-center gap-4 bg-slate-900/80 border border-white/10 px-4 py-2 rounded-2xl shadow-lg w-full max-w-sm md:w-auto shrink-0">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-[10px] text-gray-400 italic leading-tight max-w-[200px]">
          ...
          <div className="flex flex-wrap items-center justify-center xl:justify-end gap-2 text-xs font-semibold text-gray-300">
            ...
            <div className="hidden lg:flex items-center gap-1.5 bg-slate-900 border border-white/5 px-3 py-1 rounded-full font-mono text-[9px] select-none">
              <span className="text-slate-500 font-bold uppercase mr-1">AI Cycle:</span>
              
              <span className={`px-2 py-0.5 rounded transition-all duration-300 font-bold ${
                hasActiveIncidents
                ...
                Perceive{hasActiveIncidents ? " (active)" : activeActions.length > 0 ? " ✓" : ""}
              </span>
      ...
      <div className="max-w-7xl mx-auto px-6 mt-6">
        <div className="flex items-center gap-2 border-b border-white/5 pb-px">
          ...
          >
            🟢 Live Operations
            {(stadiumState?.actions_queue || []).filter(act => act.status === "pending").length > 0 && (
              <span className="ml-1.5 px-1.5 py-0.5 text-[9px] bg-emerald-500/20 text-emerald-400 rounded-full font-bold animate-bounce">
                {(stadiumState?.actions_queue || []).filter(act => act.status === "pending").length}
              </span>
            )}
          </button>
        ...
        <div className={activeTab === "studio" ? "space-y-8 block" : "hidden"}>
          <section className="glass-panel rounded-2xl p-5 border border-fifa-gold/40 shadow-[0_0_25px_rgba(234,179,8,0.08)] bg-slate-950/40">
            <div className="flex items-center gap-2 mb-3">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-gray-200 tracking-wide uppercase">Mission Simulation</h3>
                <p className="text-[10px] text-gray-400">Choose an operational scenario to calibrate AI models and stress-test routing algorithms</p>
              </div>
            </div>
