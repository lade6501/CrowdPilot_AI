...
export const UploadPanel: React.FC = () => {
  const { uploadCSV, uploadLoading, uploadResult, clearUpload, appLanguage } = useCrowdPilot();
  ...
  const loadSampleData = () => {
    ...
    const file = new File([blob], "fifa_stadium_snapshot.csv", { type: "text/csv" });
    uploadCSV(file);
  };
      ...
      <div>
        ...
        {!uploadLoading && !uploadResult && (
          <div className="space-y-4">
            ...
            <div className="p-3 bg-slate-900/60 border border-white/5 rounded-xl text-[11px] text-gray-400">
              <span className="font-bold text-gray-200 block mb-1">CSV Format Requirements:</span>
              <code className="block bg-slate-950 p-1.5 rounded font-mono text-[10px] text-fifa-gold select-all">
                gate,occupancy,queue<br />
                Gate A,82,7<br />
                Gate B,94,21<br />
                Gate C,41,3
              </code>
            ...
            >
              ...
              <span className="text-xs font-semibold text-gray-300 block">{t.upload_drop}</span>
            </div>
        ...
        {uploadLoading && (
          <div className="flex flex-col items-center justify-center py-16 text-gray-500 text-center">
            ...
            <p className="text-[10px] opacity-75 mt-0.5">{t.upload_analyzing}</p>
          </div>
        )}
          ...
          >
            ...
            <div className="grid grid-cols-3 gap-2 text-center text-xs">
              <div className="bg-slate-900/40 p-2 rounded-lg border border-white/5">
                <span className="text-[9px] text-gray-500 uppercase block">{t.twin_mode}</span>
              ...
              <div className="bg-slate-900/40 p-2 rounded-lg border border-white/5">
                <span className="text-[9px] text-gray-500 uppercase block">{t.rec_safety}</span>
                <span className="font-extrabold text-gray-200">{uploadResult.analysis.safety_index}/10</span>
              </div>
              <div className="bg-slate-900/40 p-2 rounded-lg border border-white/5">
                <span className="text-[9px] text-gray-500 uppercase block">{t.rec_efficiency}</span>
                <span className="font-extrabold text-gray-200">{uploadResult.analysis.efficiency_score}%</span>
              </div>
            </div>

            {}
            <div className="space-y-1">
              <span className="text-[10px] text-gray-400 uppercase font-semibold">{t.upload_results}</span>
              <div className="max-h-[110px] overflow-y-auto border border-white/5 rounded-lg bg-slate-950/60">
                <table className="w-full text-[11px] text-left">
                  ...
                  <tbody className="divide-y divide-white/5 text-gray-300">
                    {Object.entries(uploadResult.parsed_gates).map(([gName, gate]: [string, any]) => (
                      <tr key={gName}>
                        <td className="p-2 font-medium">{gName}</td>
                        <td className="p-2">
                          <span className={gate.occupancy >= 90 ? "text-red-400 font-bold" : gate.occupancy >= 75 ? "text-amber-400" : "text-emerald-400"}>
                            {gate.occupancy}%
                          </span>
                        </td>
